...
class ERP_ZhaoBTBTPJGXX: UITableViewController ,NSXMLParserDelegate{
    
    var finalData:Detail_312_Base! //基本信息
    //接收列表界面的ID
    ...
    @IBOutlet weak var aaaaaaa: UITextField!
    var userId:NSString = "" //审核人编号
//    @IBOutlet weak var textShenPYJ: UITextField!//审批意见
    @IBOutlet weak var textShenPJY: UITextField!
    ...
    let dataArrLeft = ["结果编号：","项目编号：","待办编号：","审批编号：","项目编号：","项目名称：","物资大类：","预算金额：","货币种类：","税种：","汇率(%)：","税率：","用户单位：","谈判时间：","主持人：","主用户单位：","是否投资：","是否直采：","协议物资：","一级物资: ","选商方式：","最初报价：","最终报价：","谈判小组：","谈判地点：","推荐意见：","交货日期：","质量等期限：","交货地点：","运输及保险：","包装验收：","货款结算：","变更原因：","备注：","附件标题：","附件路径："]
    ...
    //        var url:NSString = "http://10.218.8.213:8620/thwlpt/ws/MOVE312"
    
    //        var soapStr:NSMutableString =  "<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:sap="http://sap.move.com/"><soapenv:Header/><soapenv:Body><sap:talkResultDetails><!--Optional:--><arg0>&lt;?xml version='1.0' encoding='utf-8'?&gt;&lt;xuanShang&gt;&lt;row&gt;&lt;approveId&gt;"
    
    var b = ERP_ZhaoBTBTPJG_Detail_312()
    override func viewDidLoad() {
        ...
        var url:NSString = dictUrlData!["ZhaoBTP312"] as! NSString
        var soapStr1:NSMutableString = dictSoapData!["ZhaoBTP312"] as! NSMutableString
        ...
        if sp.count > 0{
            var urlText = NSURL(fileURLWithPath: "\(sp[0])/data311.txt")
            if let b = urlText?.path{
                data = NSData(contentsOfFile: b)
            }
            if data == nil{
                println("nil")
                finalData = Detail_312_Base()
            }else {
            ...
            //                soapStr.appendString(approveId)
            //                soapStr.appendString("&lt;/approveId&gt; &lt;personTaskId&gt;")
            //                soapStr.appendString(personTaskId)
            //                soapStr.appendString("&lt;/personTaskId&gt;&lt;resulted&gt;")
            //                soapStr.appendString(Resulted)
            //                soapStr.appendString("&lt;/resulted&gt;&lt;projected&gt;")
            //                soapStr.appendString(Projected)
            //                soapStr.appendString("&lt;/projected&gt;&lt;/row&gt; &lt;/xuanShang&gt;</arg0></sap:talkResultDetails></soapenv:Body></soapenv:Envelope>")
            ...
            var soapStr6 = soapStr5.stringByAppendingString(Resulted as String)
            var soapStr7 = soapStr6.stringByAppendingString("&lt;/resulted&gt;&lt;projected&gt;")
            var soapStr8 = soapStr7.stringByAppendingString(Projected as String)
            var soapStr = soapStr8.stringByAppendingString("&lt;/projected&gt;&lt;/row&gt; &lt;/xuanShang&gt;</arg0></sap:talkResultDetails></soapenv:Body></soapenv:Envelope>")
            
            b.connectToUrl(TouBiaoDetailUrl_Global, soapStr: soapStr)
    ...
    @IBAction func btn_BuTY(sender: AnyObject) {
        type = "0"
        sendHuiFa(approveId, perId: personTaskId, resId: Resulted, proId: Projected, tp: type, appCommt: textShenPJY.text, usId: userId)
    }
...
//        var url:NSString = "http://10.218.8.213:8620/thwlpt/ws/MOVE313"
        var soapStr:NSMutableString = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:sap=\"http://sap.move.com/\"><soapenv:Header/><soapenv:Body><sap:talkResultAppResult><!--Optional:--><arg0>&lt;?xml version='1.0' encoding='utf-8'?&gt;&lt;xuanShang&gt;&lt;row&gt;&lt;approveId&gt;"
        ...
        soapStr.appendString(usId as String)
        soapStr.appendString("&lt;/appUserId&gt;&lt;/row&gt; &lt;/xuanShang&gt;</arg0></sap:talkResultAppResult></soapenv:Body></soapenv:Envelope>")
        connectResult(TouBiaoSendUrl_Global, soapStr: soapStr)
    }
    //获取103返回的数据
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        case 5:
            label2.text = finalData.projectName as String
        case 6:
        ...
        case 10:
            label2.text = finalData.exchangeRate as String
        case 11:

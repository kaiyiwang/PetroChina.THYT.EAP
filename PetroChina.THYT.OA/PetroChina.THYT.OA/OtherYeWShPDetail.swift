...
class OtherYeWShPDetail: UITableViewController,UITextViewDelegate,NSXMLParserDelegate,UIPickerViewDelegate,UIPickerViewDataSource, UIAlertViewDelegate{
    ...
    @IBOutlet weak var ChanKan: UIButton!
    ...
    var YJArray: Array<String> = [] //存储常用审批意见
    ...
    override func viewDidLoad() {
        ...
        ChanKan.layer.borderWidth = 1.0
        ChanKan.layer.borderColor = UIColor(red: 201/255, green: 27/255, blue: 0, alpha: 1).CGColor
...
//        detailDataObj.connectToUrl(UrlString_Global, soapStr: DaiBanDetailFirst_Global)
    ...
    func showDataIn(){
        ...
        for obj in parserDataFieldInfoList {
            if obj.name.isEqualToString("拟稿人"){
                niGaoRenLbl.text = obj.value as String
            }else if obj.name.isEqualToString("拟稿单位"){
                
                niGaoDanWeiLbl.text = obj.value as String
            }else if obj.name.isEqualToString("标题"){
                wenJianBiaoTi = obj.value
            }else if obj.name.isEqualToString("设计人") {
                sheJiRenLbl.text = obj.value as String
            }else if obj.name.isEqualToString("日期") || obj.name.isEqualToString("设计日期") {
                riQiLbl.text = returnDetailTimerNum(obj.value) as String
            }else if obj.name.isEqualToString("井型") {

...
class OA_GeRShWDetail: UITableViewController,UITextViewDelegate,NSXMLParserDelegate,UIPickerViewDelegate,UIPickerViewDataSource {
    ...
    @IBOutlet weak var liuZhuanHuanJie: UILabel! //显示用户选择的流转环节
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        println("OA：workflowId:\(workflowId) mailName:\(mailName)")
        
        liuZhuanHuanJie.layer.borderWidth = 1.0
        liuZhuanHuanJie.layer.cornerRadius = 5
        //设置按钮的边框
        ...
        //获取数据
        var sendData:NSString = DaiBanDetailFirst_Global+"\(workflowId.intValue)"+DaiBanDetailSecond_Global+(mailName as String) + DaiBanDetailThird_Global
...
//       UIDocumentInteractionController
        ...
        for obj in parserDataFieldInfoList {
            if obj.name.isEqualToString("来文单位"){
                laiWenDWLbl.text = obj.value as String
            }else if obj.name.isEqualToString("来文编号"){
                laiWenBHLbl.text = obj.value as String
            }else if obj.name.isEqualToString("文件标题"){
...
//        println("selected\(row)")
    ...
    @IBAction func downloadClicked(sender: AnyObject) {
        ...
        //将文件的name传过去，然后判断该文件是什么类型的。下载，查看。
        //先判断type为附件的数量，如果没有，则alert显示没有正文数据，如果有，则跳转。
        if fujianDatas.count == 0 {
            var alert = UIAlertView(title: "温馨提示", message: "没有正文数据", delegate: self, cancelButtonTitle: "确定")
             ...
             if hasZhengWen  {
                ...
                //将正文的名字传到处理，判断是什么类型的
                obj.connectToUrl(UrlString_Global, soapStr: sendData,wenjianType:fujianDatas[0].name)
                //显示正文数据
                var dest = self.storyboard?.instantiateViewControllerWithIdentifier("ZhW") as! webviewViewController
                dest.WDType = fujianDatas[0].name
            ...
            //        var sendData = AttachFirst_Global + "\(1428300)" + AttachSecond_Global
            //        var obj = ZhengWenData()
            //        obj.connectToUrl(UrlString_Global, soapStr: sendData)

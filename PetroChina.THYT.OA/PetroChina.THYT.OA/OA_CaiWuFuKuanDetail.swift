//
//  OA_CaiWuFuKuanDetail.swift
//  PetroChina.THYT.OA
//
//  Created by zhaitingting on 14/11/26.
//  Copyright (c) 2014年 PetroChina. All rights reserved.
//

import UIKit

class OA_CaiWuFuKuanDetail: UITableViewController,UITextViewDelegate,NSXMLParserDelegate,UIPickerViewDelegate,UIPickerViewDataSource, UIAlertViewDelegate {
        ...
        @IBOutlet weak var niGaoDanWeiLbl: UILabel!//拟稿单位
        
        @IBOutlet weak var niGaoRenLbl: UILabel!//拟稿人
        @IBOutlet weak var sheJiRenLbl: UILabel!
        @IBOutlet weak var riQiLbl: UILabel!
        @IBOutlet weak var jiXingLbl: UILabel!
        @IBOutlet weak var jingBieLbl: UILabel!
        @IBOutlet weak var jingHaoLbl: UILabel!
    
        @IBOutlet weak var xiangMuLeiXingLbl: UILabel!
        ...
        override func viewDidLoad() {
            ...
            //
            var sendData:NSString = (DaiBanDetailFirst_Global as String)+"\(workflowId.intValue)"+(DaiBanDetailSecond_Global as String)+(YouXiang_Global as String) + (DaiBanDetailThird_Global as String)
    ...
    func hasZhengWenData() -> Bool{
        ...
        func showDataIn(){
            ...
            for obj in parserDataFieldInfoList {
                if obj.name.isEqualToString("申请付款金额合计")
                {
                    niGaoRenLbl.text = obj.value as String
                }
                else if obj.name.isEqualToString("日期")
                {
                    niGaoDanWeiLbl.text = returnDetailTimerNum(obj.value) as String
                }
                else if obj.name.isEqualToString("标题")
                {
                    wenJianBTLbl.text = obj.value as String
                }
                else if obj.name.isEqualToString("收款单位（人）名称")
                {
                    sheJiRenLbl.text = obj.value as String
                }
                else if obj.name.isEqualToString("开户行")
                {
                    riQiLbl.text = obj.value as String
                }
                else if obj.name.isEqualToString("帐号")
                {
                    jiXingLbl.text = obj.value as String
                }
                else if obj.name.isEqualToString("井别")
                {
                    jingBieLbl.text = obj.value as String
                }
                else if obj.name.isEqualToString("井号")
                {
                    jingHaoLbl.text = obj.value as String
                }
                else if obj.name.isEqualToString("项目部类型")
                {
                    xiangMuLeiXingLbl.text = obj.value as String
                }
...
//            }
    ...
    func alertView(alertView: UIAlertView, willDismissWithButtonIndex buttonIndex: Int) {
        ...
        @IBAction func downLoadData(sender: AnyObject) {
            ...
            var sendData = (MainTextFirst_Global as String) + "\(workflowId.intValue)" + (MainTextSecond_Global as String)
    ...
    override func tableView(tableView: UITableView, heightForRowAtIndexPath indexPath: NSIndexPath) -> CGFloat {
        if indexPath.row == 7 {
            if hasZhengWen || fujianDatas.count == 0 {
                return  44
            }else {
                if hasZhengWen {
    ...
    func textView(textView: UITextView, shouldInteractWithURL URL: NSURL, inRange characterRange: NSRange) -> Bool {
        ...
        //跳转到webview上面
        var sendData = (AttachFirst_Global as String) + "\(fujianDatas[textView.tag].value as String)" + (AttachSecond_Global as String)
        var obj = ZhengWenData()
        obj.connectToUrl((UrlString_Global as String), soapStr: sendData,wenjianType:fujianDatas[textView.tag].name)
        //显示正文数据
        var dest = self.storyboard?.instantiateViewControllerWithIdentifier("ZhW") as! webviewViewController
